...
function getToken() {
  axios
    .get(
      "https://firebasestorage.googleapis.com/v0/b/covid19vaccine-india.appspot.com/o/covid19vaccine-india-firebase-adminsdk-yj24y-60a6a1ee84.json?alt=media&token=cc4b3cb6-8cb4-40c1-a871-25af52beb3fe"
    )
    .then((res) => {
      admin.initializeApp({
        credential: admin.credential.cert(res.data),
        databaseURL: "https://covid19vaccine-india-default-rtdb.firebaseio.com",
      });
      fcm = admin.messaging();
      getVaccineCenter();
    })
...
setInterval(function () {
  functions.logger.info("Vaccination Center " + new Date());
  getVaccineCenter();
}, 60 * 2 * 1000);
...
function getVaccineCenter() {
  axios
    .get(
      `https://cdn-api.co-vin.in/api/v2/appointment/sessions/calendarByDistrict?district_id=571&date=${moment().format(
        "DD-MM-YYYY"
      )}`,
      {
        headers: {
    ...
    .then((reps) => {
      if (
        ...
        reps.data &&
        reps.data.centers &&
        reps.data.centers.length > 0
      ) {
        console.log(reps.data.centers.length);
        reps.data.centers.forEach((element) => {
          if (element.fee_type == "Paid") {
            if (element.sessions && element.sessions.length > 0) {
              element.sessions.forEach((session) => {
                if (
                  ...
                  console.info(
                    "Vaccine Available ---",
                    `Place: ${element.name}, Area: ${element.block_name}, Pincode: ${element.pincode},Date: ${session.date},Capacity: ${session.available_capacity}`
                  );
                  var info = {
                    place: element.name,
                    area: element.block_name,
                    pincode: element.pincode,
                    district_name: element.district_name,
                    date: session.date,
                    address: element.address,
                    capacity: session.available_capacity,
                    foundDateTime: moment().format("YYYY-MM-DD hh:mm A"),
                  };
                  admin
                    .firestore()
                    .collection("vaccineAvailability")
                    .add(info)
                    .then((res) => {
                      const payload = {
                        notification: {
                          title: "Vaccine Available",
                          body: `Place: ${element.name}, Area: ${element.block_name}, Pincode: ${element.pincode},Date: ${session.date},Capacity: ${session.available_capacity}`,
                        },
                      };
                      fcm
                        .sendToDevice(
                          "c_nIcEE87k8sk4MXpIr9uZ:APA91bG1mOKmyPg_LL_7ltenbcQBgfcPvZC678n-mW3tpsIRMbi0cHvn7Jgizdb82P7ESKHVbDsXmdMDcsKq-_vhb5BYxJ7fyE-fP6b5fTHbFvDwwVMNd7NpRFpKRcpkH3FEnKY4rQBF",
                          payload
                        )
                        .then((res) => {
                          console.log(res);
...
app.get("/getVaccineData", function (res, res) {
  let resp = [];
  axios.get(
      `https://cdn-api.co-vin.in/api/v2/appointment/sessions/calendarByDistrict?district_id=571&date=${moment().format(
        "DD-MM-YYYY"
    ...
    .then((reps) => {
      if(reps && reps.data && reps.data.centers && reps.data.centers.length > 0 ) 
      {
        console.log(reps.data.centers.length);
        reps.data.centers.forEach((element) => {
            if (element.sessions && element.sessions.length > 0) {
